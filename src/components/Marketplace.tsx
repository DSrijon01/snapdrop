            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8 items-center justify-between sticky top-24 z-20 bg-black/80 backdrop-blur-xl p-4 rounded-xl border border-red-900/20 shadow-xl">
                ...
                <div className="relative w-full md:w-96">
                    ...
                    <input
                        ...
                        className="w-full bg-[#1b1b1b] border border-white/5 rounded-xl py-3 pl-10 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-600/50 focus:border-transparent transition-all"
                        value={searchTerm}
                ...
                <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    <button className="px-5 py-2.5 bg-[#1b1b1b] hover:bg-[#2a2a2a] border border-white/5 rounded-full text-sm font-semibold text-white whitespace-nowrap transition-colors flex items-center gap-2 group">
                        Price: Low to High
                     ...
                     {/* View Toggles (Visual) */}
                     <div className="flex bg-[#1b1b1b] rounded-lg p-1 border border-white/5">
                        <button className="p-2 bg-[#2a2a2a] text-white rounded-md shadow-sm border border-white/5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {filteredItems.map((item) => (
                    <motion.div
                        ...
                        className="group relative bg-[#121212] border border-white/5 rounded-2xl overflow-hidden hover:border-red-600/30 hover:shadow-[0_0_20px_rgba(220,38,38,0.15)] transition-all duration-300"
                    >
                        {/* Image */}
                        <div className="aspect-square overflow-hidden relative">
                             {item.isHot && (
                                 <div className="absolute top-3 right-3 z-10 bg-red-600/90 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-lg">
                                     Hot
                                 </div>
                             )}
                            <img 
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-black via-black/80 to-transparent flex flex-col justify-end h-1/2">
                                <button className="w-full py-3 bg-red-600 text-white font-bold text-sm rounded-xl hover:bg-red-500 transition-colors shadow-lg">
                        ...
                        <div className="p-4">
                            <div className="flex justify-between items-start mb-2">
                                <h3 className="font-bold text-white text-lg tracking-tight group-hover:text-red-500 transition-colors">{item.name}</h3>
                                <span className="text-xs font-mono text-gray-500">#{item.rank}</span>
                            </div>
                            
                            <div className="flex items-end justify-between">
                                <div>
                                    <div className="text-[10px] text-gray-500 mb-0.5 font-medium">Price</div>
                                    <div className="text-white font-bold text-lg flex items-center gap-1">
                                        {item.price} <span className="text-xs text-gray-500 font-normal">SOL</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[10px] text-gray-500 mb-0.5 font-medium">Last Sale</div>
                                    <div className="text-gray-300 font-medium text-sm">
                                        {item.lastSale || "-"} <span className="text-[10px] text-gray-600">WETH</span>
                                    </div>
                                </div>
                            </div>
                        </div>
