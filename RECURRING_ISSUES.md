...
| **4** | `Transaction was not confirmed in 30.00 seconds` (`TransactionExpiredTimeoutError`) | Listing tokens / stacks / secondary marketplace | Zero priority fee causes validators to delay processing past Anchor's 30s confirmation window; or redundant `connection.confirmTransaction()` watchdog | Attach `ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 100_000 })` and `setComputeUnitLimit({ units: 400_000 })`, remove redundant `confirmTransaction()`, and ensure `withSolanaRetry()` catches `TransactionExpiredTimeoutError` / `not confirmed in`. |
